...
REM PersonaPlex Server Launcher - Public Share (CPU Offload)
REM Created by SurAiverse - https://www.youtube.com/@suraiverse

title PersonaPlex Server (Public Share + CPU Offload) - by SurAiverse
color 0B
...
echo  ^|                                                                             ^|
echo  ^|         SERVER LAUNCHER - PUBLIC SHARE (CPU OFFLOAD + TUNNEL)               ^|
echo  ^|                                                                             ^|
...
echo  Public Share: Creates a public link using a Gradio tunnel
echo.
...
REM ================================================================================
REM Check and install accelerate (CPU offload)
REM ================================================================================
...
REM Launch server with CPU offload and public tunnel
if "!STATIC_PATH!"=="none" (
    python -m moshi.server --static none --cpu-offload --gradio-tunnel !TUNNEL_TOKEN_ARG!
) else (
    python -m moshi.server --static "!STATIC_PATH!" --cpu-offload --gradio-tunnel !TUNNEL_TOKEN_ARG!
)
