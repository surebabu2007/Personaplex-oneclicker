...
export const Conversation:FC<ConversationProps> = ({
  ...
  const sessionProgress = useMemo(() => {
    if (isOver) {
      return { value: 100, label: "Complete", step: "Complete" };
    }
    if (socketStatus === "connected") {
      return { value: 100, label: "Live", step: "Live" };
    }
    if (socketStatus === "connecting") {
      return { value: 60, label: "Connecting", step: "Connecting" };
    }
    return { value: 20, label: "Ready", step: "Ready" };
  }, [isOver, socketStatus]);
  useEffect(() => {
  ...
  return (
    ...
    <div className="main-grid h-screen max-h-screen w-screen max-w-96 md:max-w-screen-lg m-auto p-2">
      <div className="controls ppx-panel flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
        <div className="flex flex-col gap-1 text-left">
          <div className="ppx-title text-xl md:text-2xl">PersonaPlex</div>
          <div className="ppx-tagline text-xs">Simplified &amp; one-click install by SurAiverse</div>
          <div className="ppx-subtag text-[0.65rem]">NVIDIA PersonaPlex 7B</div>
        </div>
        <div className="flex-1">
          <div className="ppx-progress">
            <div className="ppx-progress-row">
              <span>Session</span>
              <span>{sessionProgress.label}</span>
            </div>
            <div className="ppx-progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={sessionProgress.value}>
              <div className="ppx-progress-bar" style={{ width: `${sessionProgress.value}%` }} />
            </div>
            <div className="ppx-progress-steps">
              <span className="active">Ready</span>
              <span className={sessionProgress.value >= 60 ? "active" : ""}>Connecting</span>
              <span className={sessionProgress.value >= 100 && !isOver ? "active" : ""}>Live</span>
              <span className={isOver ? "active" : ""}>Complete</span>
            </div>
          </div>
        </div>
        ...
        {audioContext.current && worklet.current && <MediaContext.Provider value={
          ...
          <div className="relative player h-full max-h-full w-full justify-between gap-3 md:p-10 ppx-panel">
              <ServerAudio
              ...
              {isOver && audioURL && (
                <div className="ppx-download text-sm">
                  <div>
                    <div className="text-xs uppercase tracking-[0.2em] text-[#6e5d3b]">Session Complete</div>
                    <div className="text-xs text-[#7b6a4a]">Download your conversation audio</div>
                  </div>
                  <a
                    href={audioURL}
                    download={`personaplex_audio.${getExtension("audio")}`}
                    className="ppx-button"
                  >
                    Download Audio
                  </a>
                </div>
          ...
          <div className="scrollbar player-text ppx-panel" ref={textContainerRef}>
            <TextDisplay containerRef={textContainerRef}/>
          </div>
          <div className="player-stats hidden md:block ppx-panel">
